...
def video_create(request, training_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                # VideoUploadForm é um Form simples, não ModelForm
                # Precisamos criar o objeto Video manualmente
                video = Video.objects.create(
                    ...
                    order=training.videos.count() + training.quizzes.count() + 1
                )
                
                logger.info(f"Vídeo {video.id} '{video.title}' salvo com sucesso. Processamento de metadados ocorrerá em background via signal.")
                
                # Mensagem de sucesso imediata - o processamento de metadados acontece via signal
                messages.success(request, f'Vídeo "{video.title}" recebido com sucesso! A duração e thumbnail serão processados em segundo plano.')
                
                return redirect('trainings:manage_detail', pk=training.pk)
                
            except Exception as e:
                logger.error(f"Erro ao salvar vídeo: {str(e)}", exc_info=True)
